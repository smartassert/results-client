...
use SmartAssert\ResultsClient\Tests\Integration\AbstractIntegrationTestCase;

class AddAndGetEventTest extends AbstractIntegrationTestCase
{
    public function testAddAndGetInvalidJobToken(): void
    {
        $jobToken = 'invalid token';

        self::expectExceptionObject(new InvalidJobTokenException($jobToken));

        self::$client->addAndGetEvent(
            $jobToken,
            new Event(
                1,
                'event_type',
    ...
    public function testAddAndGetSuccess(Event $event, callable $expectedEventCreator): void
    {
        $jobEvent = self::$client->addAndGetEvent(self::$user1Job->token, $event);
